...
def checkpoint_model(
    ...
    """Save interim model during experiment.

    Params:
        exp_dict: experiment dictionary.
        results_dir: base directory for experimental results.
        model: the optimized model to be serialized.
    """

    hash_id = configs.hash_dict(exp_dict)
    path = os.path.join(results_dir, hash_id, "checkpoints")
    ...
    if model is not None:
        # save a PyTorch model using torch builtins.
        if isinstance(model, torch.nn.Module):
            torch_path = os.path.join(path, f"model_at_{iteration}.pt")
            model = cast(torch.nn.Module, model)
            ...
            # pickle the model as usual.
            pkl_path = os.path.join(path, f"model_at_{iteration}.pkl")
            with open(pkl_path, "wb") as f:
                pkl.dump(model, f)
...
def load_checkpoints(
    ...
    """Load all model checkpoints from an experiment.
    ...
    for src in utils.as_list(results_dir):
        path = os.path.join(src, hash_id, "checkpoints")
    ...
    if not success:
        raise ValueError(
            f"Cannot find checkpoints for {exp_dict} in one of {results_dir}!"
        )
    ...
    for checkpoint_name in checkpoints:
        ...
        iteration_number = int(checkpoint_name.split("model_at_")[1].split(".")[0])

        if checkpoint_name[-3:] == ".pt":
            checkpoint_dict[iteration_number] = torch.load(checkpoint_path)
        else:
            with open(checkpoint_path, "rb") as f:
                checkpoint_dict[iteration_number] = pkl.load(f)
